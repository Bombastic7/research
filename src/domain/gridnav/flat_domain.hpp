...
namespace mjon661 { namespace gridnav { namespace flatlayout {
	...
	struct FourWayBase {
		...
		void getHeuristicValues(idx_t pPos, int& out_h, int& out_d) const {
			if(!Use_H)
				out_h = out_d = 0;
			
			else if(!Use_LC)
				out_h = out_d = manhat(pPos);
			
			else
				lifeCostHeuristics(pPos, out_h, out_d);
		}
	...
	struct EightWayBase {
		...
		void getHeuristicValues(idx_t pPos, cost_t& out_h, cost_t& out_d) const {
			if(!Use_H)
				out_h = out_d = 0;
				...
				out_h = std::abs(dx-dy) + min(dx, dy) * SQRT2;
				out_d = max(dx, dy);
			}
	 ...
	 * 
	 * 	void getHeuristicValues(idx_t pos, cost_t& out_h, cost_t& out_d);
	 * 
	...
	struct GridNav_Dom : public DomBase {
		...
		Edge createEdge(State const& pState, Operator op) const {
			...
			this->getHeuristicValues(pState.pos, edgeState.h, edgeState.d);
